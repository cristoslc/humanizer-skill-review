...
The judge is prompted with the rubric (`comparison/rubric.md`), the candidate's SKILL.md, the original input, and the rewritten output. It returns strict JSON scoring all 10 criteria. Per-fixture verdicts are aggregated by modal rating into `scores.json`. The aggregation code is in `tests/harness/score-outputs.py`.
