...
2. The candidate's full repo context (file tree, README, LICENSE, CHANGELOG, CI workflows, test files, package manifest, git history) — use this for repo-level criteria (maturity, structure, portability).
3. The original AI-slop input text (one fixture from a fixed corpus).
4. The rewritten output produced by an agent operating under that skill.
5. The scoring rubric with 10 criteria.

Score the skill on ALL 10 rubric criteria. For each criterion, assign an integer from 1 to 3 (3 is strongest, 1 is weakest), or 0 if the criterion does not apply. Also give a one-sentence rationale for each score, grounded in evidence from the SKILL.md, the repo context, and the output.
...
Be critical. Do not default to `2` when evidence supports `3` or `1`. Ground every rationale in something concrete you can point to in the SKILL.md, the repo context, or the output. For repo-level criteria (maturity, skill_structure, portability), use the repo context section — do not judge maturity from the SKILL.md alone.
